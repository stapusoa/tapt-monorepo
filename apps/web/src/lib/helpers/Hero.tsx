...
<div className=" flex flex-col gap-6 z-20 py-12 px-8 max-w-6xl w-full text-left">
            {content.badge && (
<span className="self-start px-3 py-1 inline-block rounded-full bg-neutral-400 text-secondary-dark text-sm font-semibold mb-2">
                {content.badge}
              </span>
            )}
            ...
            {content.buttons && content.buttons.length > 0 && (
              <div className={HERO_CONTENT_STYLES.buttons}>
                {content.buttons.map((btn, idx) => (
                  <button key={idx} onClick={btn.onClick} className={`${HERO_CONTENT_STYLES.button} ${btn.className}`}>
                    {btn.text}
                  </button>
                ))}
              </div>
