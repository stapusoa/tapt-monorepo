...
import { cardStyles, getImageClasses, getContentAlignH, getContentAlignV } from "@/components/ui/card/styles"
...
export function ProductCard({ product, size = "md" }: ProductCardProps) {
  const { title, price, image, description, category, tags = [] } = product
    ...
    >
      {description && (
        <CardContent className="pt-2">
          <p className="text-sm text-body line-clamp-3">{description}</p>
        </CardContent>
      )}
      <CardFooter>
        {tags.slice(0, 3).map((tag) => (
          <Badge key={tag} variant="filled" color="neutral" className="text-xs">
            {tag}
          </Badge>
        ))}
        {tags.length > 3 && (
